...
public sealed class ObjectPool : MonoBehaviour
{
    ...
    [SerializeField]
    private int defaultMassiveAddedPoolSize = 10;
    [SerializeField]
    private List<GameObject> massiveAddedObjects = new List<GameObject>();
    ...
    {
        for (int i = 0; i < massiveAddedObjects.Count; i++)
        {
            startupPools.Add(new StartupPool()
            {
                size = defaultMassiveAddedPoolSize,
                prefab = massiveAddedObjects[i]
            });
        }
        massiveAddedObjects.Clear();
    }
    ...
    public static T Spawn<T>(string name, Transform parent, Vector3 position, Quaternion rotation) where T : Component
    {
        return Spawn(name, parent, position, rotation).GetComponent<T>();
    }
    public static T Spawn<T>(string name, Vector3 position, Quaternion rotation) where T : Component
    {
        return Spawn(name, null, position, rotation).GetComponent<T>();
    }
    public static T Spawn<T>(string name, Transform parent, Vector3 position) where T : Component
    {
        return Spawn(name, parent, position, Quaternion.identity).GetComponent<T>();
    }
    public static T Spawn<T>(string name, Vector3 position) where T : Component
    {
        return Spawn(name, null, position, Quaternion.identity).GetComponent<T>();
    }
    public static T Spawn<T>(string name, Transform parent) where T : Component
    {
        return Spawn(name, parent, Vector3.zero, Quaternion.identity).GetComponent<T>();
    }
    public static T Spawn<T>(string name) where T : Component
    {
        return Spawn(name, null, Vector3.zero, Quaternion.identity).GetComponent<T>();
    }
    ...
    public static GameObject Spawn(string name, Transform parent, Vector3 position, Quaternion rotation)
    {
        ...
        if (pools != null && pools.Count > 0)
            for (int i = 0; i < pools.Count; ++i)
                if (pools[i].prefab.name.Equals(name)) prefab = pools[i].prefab;
